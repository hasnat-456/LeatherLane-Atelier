...
async function loadSliderSettings() {
    try {
        ...
        if (res.ok) {
            const data = await res.json();
            heroSliderImages = data.heroSliderImages ? JSON.parse(data.heroSliderImages) : [];
            craftSliderImages = data.craftSliderImages ? JSON.parse(data.craftSliderImages) : [];
            renderSliders();
        }
    } catch (e) {
...
function renderSliders() {
    ...
    heroSliderImages.forEach((img, i) => {
        heroContainer.innerHTML +=             <div style="display:flex; justify-content:space-between; align-items:center; background:#f9f9f9; padding:5px; border:1px solid #ddd;">
                <span style="font-size:0.85rem; word-break:break-all;">\</span>
                <button type="button" onclick="removeSliderImage('Hero', \)" style="color:red; background:none; border:none; cursor:pointer;">?</button>
            </div>
        \;
    });
    ...
    craftSliderImages.forEach((img, i) => {
        craftContainer.innerHTML +=             <div style="display:flex; justify-content:space-between; align-items:center; background:#f9f9f9; padding:5px; border:1px solid #ddd;">
                <span style="font-size:0.85rem; word-break:break-all;">\</span>
                <button type="button" onclick="removeSliderImage('Craft', \)" style="color:red; background:none; border:none; cursor:pointer;">?</button>
            </div>
        \;
    });
}
...
async function uploadSliderImage(type) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            fileInput.value = '';
            renderSliders();
            alert("Uploaded successfully! Don't forget to click 'Save Sliders'.");
        } else {
