            ...
            if (System.IO.File.Exists(physicalPath))
            {
                ...
                try 
                {
                    ...
                    if (path.Equals("/home.html", StringComparison.OrdinalIgnoreCase))
                    {
                        if (allProducts.Any())
                        {
                            ...
                            foreach(var p in allProducts.Take(4))
                            {
                                ...
                                sb.Append($"<div class='product-card'><a href='/product-detail?id={p.Id}' style='text-decoration: none; color: inherit;'><img src='{rawImg}' class='product-img' decoding='async' onerror=\"this.onerror=null;this.src='https://via.placeholder.com/300x250';\" alt='{System.Net.WebUtility.HtmlEncode(p.Name)}' style='width: 100%; height: 250px; object-fit: cover;'><div class='product-info'><div class='product-category'>{p.Category}</div><h3 class='product-title'>{p.Name}</h3><div class='product-price'>Rs {p.Price:N0}</div></div></a></div>");
                            }
                        ...
                        foreach(var cat in categories)
                        {
                            ...
                            {
                                var rawImg = string.IsNullOrEmpty(p.Thumbnail) ? (p.Images != null && p.Images.Count > 0 ? p.Images[0] : "https://via.placeholder.com/300x250") : p.Thumbnail;
                                sb.Append($"<div class='product-card'><a href='/product-detail?id={p.Id}' style='text-decoration: none; color: inherit;'><img src='{rawImg}' class='product-img' loading='lazy' decoding='async' onerror=\"this.onerror=null;this.src='https://via.placeholder.com/300x250';\" alt='{System.Net.WebUtility.HtmlEncode(p.Name)}' style='width: 100%; height: 250px; object-fit: cover;'><div class='product-info'><div class='product-category'>{p.Category}</div><h3 class='product-title'>{p.Name}</h3><div class='product-price'>Rs {p.Price:N0}</div></div></a></div>");
                            }
                            sb.Append("</div></section>");
                        }
                        ...
                        if (recentStories.Any())
                        {
                            ...
                            foreach(var story in recentStories)
                            {
                                ...
                                <div style='background: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05); transition: transform 0.3s; cursor: pointer;' onmouseover=""this.style.transform='translateY(-5px)'"" onmouseout=""this.style.transform='translateY(0)'"" onclick=""window.location.href='/story?id={story.Id}'"">
                                    <img src='{sImg}' style='width: 100%; height: 250px; object-fit: cover;' loading='lazy' decoding='async' onerror=""this.onerror=null;this.src='https://via.placeholder.com/400x250';"">
                    ...
                    else if (path.Equals("/products.html", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        {
                            var rawImg = string.IsNullOrEmpty(p.Thumbnail) ? (p.Images != null && p.Images.Count > 0 ? p.Images[0] : "https://via.placeholder.com/300x250") : p.Thumbnail;
                            sb.Append($"<div class='product-card' data-category='{p.Category?.ToLower()}' data-price='{p.Price}'><a href='/product-detail?id={p.Id}' style='text-decoration: none; color: inherit;'><img src='{rawImg}' class='product-img' loading='lazy' decoding='async' onerror=\"this.onerror=null;this.src='https://via.placeholder.com/300x250';\" style='width: 100%; height: 250px; object-fit: cover;'><div class='product-info'><div class='product-category'>{p.Category}</div><h3 class='product-title'>{p.Name}</h3><div class='product-price'>Rs {p.Price:N0}</div></div></a></div>");
                        }
