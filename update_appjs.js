...
async function loadSocialLinks() {
    try {
        ...
        if (response.ok) {
            ...
            updateLink('WhatsApp', settings.whatsAppUrl);

            // Set up a global floating WhatsApp button if configured
            if (settings.whatsAppUrl) {
                const floatBtn = document.getElementById('floating-whatsapp');
                if (floatBtn) {
                    floatBtn.href = settings.whatsAppUrl;
                } else {
                    const waBtn = document.createElement('a');
                    waBtn.id = 'floating-whatsapp';
                    waBtn.href = settings.whatsAppUrl;
                    waBtn.target = "_blank";
...
if (!code.includes('loadSocialLinks')) {
    code += jsAddition;
    fs.writeFileSync('front/js/app.js', code, 'utf8');
    console.log("Updated app.js with dynamic social links and WhatsApp button");
}
